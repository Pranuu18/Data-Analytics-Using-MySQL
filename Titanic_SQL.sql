...
-- select count(PassengerId) from titanic where survived = 1;
...
-- select name, max(fare) from titanic group by name order by max(fare) desc;
...
-- select  name, max(age) from titanic group by name order by max(age) desc limit 1;
...
select Sex, count(*) as num_survived from titanic where Survived = 1 group by Sex order by Sex asc;

-- 13. Find the name, age, and fare of the oldest passenger who survived.
-- select name , fare, age from titanic where (select max(age) from titanic where  survived = 1) and survived = 1;
...
select name , age from titanic where age = (select min(age) from titanic where sex = 'female' and survived = 1 and Pclass = 3) and  sex = 'female' and survived = 1 and Pclass = 3;
